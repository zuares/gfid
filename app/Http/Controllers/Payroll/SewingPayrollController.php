...
class SewingPayrollController extends Controller
{
    ...
    public function slip(PieceworkPayrollPeriod $period, $employeeId): View
    {
        ...
        return view('payroll.sewing.slip', [
            'period' => $period,
            'employee' => $employee,
            'lines' => $lines,
            'totalQty' => $totalQty,
    ...
    public function slipAll(PieceworkPayrollPeriod $period): View
    {
        if ($period->module !== 'sewing') {
            abort(404);
        }
        ...
        if ($lines->isEmpty()) {
            abort(404, 'Tidak ada data payroll untuk periode ini.');
        }
        ...
        return view('payroll.sewing.slip_all', [
            'period' => $period,
     ...
     */
    /**
     * Regenerate: hitung ulang di periode yang sama (in-place)
     */
    public function regenerate(PieceworkPayrollPeriod $period): RedirectResponse
    {
        ...
        // Regenerate di periode yang sama (OPTION A - in-place)
        $period = SewingPayrollGenerator::generate(

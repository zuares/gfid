...
Route::prefix('payroll/cutting')
    ...
    ->middleware(['auth']) // sesuaikan middleware-mu
    ->group(function () {
        ...
        Route::get('/{period}/slip/{employee}',
            [CuttingPayrollController::class, 'slip'])
            ->name('slip');
    });

Route::prefix('payroll/piece-rates')
    ->name('payroll.piece_rates.')
    ->middleware(['auth']) // sesuaikan middleware
    ->group(function () {
...
Route::prefix('payroll/sewing')->name('payroll.sewing.')->group(function () {
    ...
    Route::get('/{period}/slip/{employee}',
        [SewingPayrollController::class, 'slip'])
        ->name('slip');
    // SLIP SEMUA OPERATOR — SEWING
    Route::get('/{period}/slip-all',
        [SewingPayrollController::class, 'slipAll'])
        ->name('slip_all');

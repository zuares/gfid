...
@section('title', 'Payroll • Sewing Periode ' . $period->period_start . ' s/d ' . $period->period_end)
...
@section('content')
    <div class="page-wrap">
        ...
        <div class="d-flex justify-content-between align-items-start mb-3">
            <div>
                <a href="{{ route('payroll.sewing.index') }}" class="btn btn-link px-0 small mb-1">
            ...
            <div class="d-flex flex-column flex-sm-row gap-2">
                {{-- Slip semua operator selalu boleh diakses --}}
                <a href="{{ route('payroll.sewing.slip_all', $period) }}" class="btn btn-outline-primary btn-sm w-100">
                    Slip Semua Operator
                </a>

                @if ($period->status !== 'final')
                    <form action="{{ route('payroll.sewing.regenerate', $period) }}" method="POST"
                    ...
                    <form action="{{ route('payroll.sewing.finalize', $period) }}" method="POST"
                                ...
                                <tr>
                                    <td>
                                        ...
                                        @if (!empty($row['employee_id']))
                                            <a href="{{ route('payroll.sewing.slip', [$period, $row['employee_id']]) }}"
                                                class="badge bg-primary-subtle text-primary border border-primary-subtle badge-slip ms-1">
                                                Slip
                                            </a>
                                        @endif
                                    </td>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="help-text mb-3">
                    Breakdown per operator × kategori × item (berdasarkan Sewing Return OK).
                </p>

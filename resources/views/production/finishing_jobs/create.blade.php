        ...
        <div class="card p-3 p-md-3 mb-3">
            <div class="page-header">
                <div class="page-title-wrap">
                    ...
                    <div>
                        <h1 class="page-title">Finishing Job Baru</h1>
                        <div class="page-subtitle">
                            Proses hasil jahit (WIP-FIN) menjadi barang jadi, dengan input reject yang sederhana.
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-3 mt-1">
                    ...
                    <div class="col-md-8">
                        ...
                        <textarea name="notes" class="form-control form-control-sm" rows="2"
                            placeholder="Misal: Finishing harian, shift pagi.">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card p-3 p-md-3 mb-3">
                <div class="section-header">
                    <div>
                        ...
                        <div class="help">
                            Pilih bundle yang siap finishing. Qty In & Qty OK otomatis dari saldo WIP-FIN.
                            Kamu hanya perlu mengisi Qty Reject (jika ada) dan alasan reject.
                        </div>
                    </div>
                        ...
                        <tbody>
                            @php
                                ...
                                $currentUserLabel = $currentUser
                                    ? $currentUser->name ?? ($currentUser->email ?? 'User #' . $currentUser->id)
                                    : null;
                            @endphp
                                    ...
                                    <tr class="line-row">
                                        ...
                                        <td data-label="Operator">
                                            @if ($currentUser)
                                                ...
                                                {{-- TODO: kalau operator_id = employee_id, ganti ke $currentUser->employee_id --}}
                                                <input type="hidden" name="lines[{{ $i }}][operator_id]"
                                                    value="{{ $currentUser->id }}">
                                            @else
                                                <div class="small text-muted">User belum login</div>
                                            @endif
                                        </td>
                                ...
                                <tr class="line-row">
                                    ...
                                    <td data-label="Operator">
                                        @if ($currentUser)
                                            ...
                                            {{-- TODO: kalau operator_id = employee_id, ganti ke $currentUser->employee_id --}}
                                            <input type="hidden" name="lines[0][operator_id]"
                                                value="{{ $currentUser->id }}">
                                        @else
                                            <div class="small text-muted">User belum login</div>
                                        @endif
        ...
        (function() {
            ...
            function addLineRow() {
                ...
                newRow.querySelectorAll('input').forEach(input => {
                    if (input.type === 'hidden') {
                        ...
                        if (input.name && input.name.includes('[wip_balance]')) {
                            // biarkan, nanti diisi lagi saat pilih bundle
                            input.value = '0';
                        } else {
                            input.value = '';
                        }
                    ...
                    if (['number', 'text'].includes(input.type)) {
                        if (input.classList.contains('qty-reject-input')) {
                            input.value = 0; // reject default 0
                        } else {
                            input.value = 0;
                        }
                    }
                ...
                newRow.querySelectorAll('select').forEach(select => {
                    if (select.classList.contains('bundle-select')) {
